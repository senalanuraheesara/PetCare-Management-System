      ...
      <View style={styles.header}>
        <SafeAreaView>
          <View style={styles.headerRow}>
            ...
            <Text style={styles.headerTitle}><FontAwesome5 name="pills" size={20} color="#FFF" /> Medication & Prescriptions</Text>
            <View style={{ width: 40 }} />
          </View>
        </SafeAreaView>
      </View>
        ...
        <ScrollView contentContainerStyle={styles.list}>
          {records.length === 0 && (
            <View style={styles.emptyCard}>
              <FontAwesome5 name="pills" size={48} color="#CCC" style={{ marginBottom: 10 }} />
              <Text style={styles.emptyTitle}>No medication records</Text>
              <Text style={styles.emptySub}>Your vet will add medication plans after your appointment.</Text>
            </View>
          )}
          {records.map(r => (
            <View key={r._id} style={styles.card}>
              <View style={styles.cardTop}>
                <View style={styles.iconBox}><FontAwesome5 name="pills" size={24} color="#5EBFA4" /></View>
                <View style={{ flex: 1, marginLeft: 12 }}>
                  <Text style={styles.cardTitle}>{r.medicationName || 'Medication'}</Text>
                  <Text style={styles.cardSub}><FontAwesome5 name="prescription-bottle" size={12} color="#888" /> Dosage: {r.dosage}</Text>
                  <Text style={styles.cardSub}><FontAwesome5 name="clock" size={12} color="#888" /> Frequency: {r.frequency}</Text>
                  <Text style={styles.cardSub}><FontAwesome5 name="calendar-day" size={12} color="#888" /> Started: {formatDate(r.startDate)}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  badgeDone: { backgroundColor: '#F5F5F5' },
